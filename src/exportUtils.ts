...
export function exportCSV(stats: PlayerStats[], match: MatchState, lang: string = 'en') {
  ...
  const rows = stats.map((s) => {
    const fundCols = fundamentals.flatMap((f) => {
      ...
      return [fs.pp, fs.p, fs.m, fs.eq, fs.total, fs.positivity];
    });
    const newMetrics = fundamentals.flatMap((f) => {
      const fs = s.fundamentals[f];
      return [fs.errorRate, fs.netEfficiency];
    });
    return [
...
export function exportPDF(stats: PlayerStats[], match: MatchState, lang: string = 'en') {
  ...
  fundamentals.forEach((fund) => {
    const head = [['No.', 'Name', '++', '+', '-', '=', 'Tot', 'Pos%', 'ER%', 'NE%']];
    const body = stats
      .filter((s) => s.fundamentals[fund].total > 0)
      .map((s) => {
        const fs = s.fundamentals[fund];
        return [s.playerNumber, s.playerName, fs.pp, fs.p, fs.m, fs.eq, fs.total, `${fs.positivity}%`, `${fs.errorRate}%`, `${fs.netEfficiency}%`];
      });
  ...
  const totHead = [['No.', 'Name', 'Role', '++', '+', '-', '=', 'Total', 'Pos%', 'ER%', 'NE%']];
  const totBody = stats
    ...
    .map((s) => {
      ...
      return [s.playerNumber, s.playerName, roleLabels[s.playerRole] || s.playerRole, s.totals.pp, s.totals.p, s.totals.m, s.totals.eq, s.totals.total, `${overallPositivity}%`, `${overallErrorRate}%`, `${overallNetEfficiency}%`];
    });
  ...
  if (activePlayers.length > 0) {
    ...
    for (const s of activePlayers) {
      ...
      for (const f of fundamentals) {
        ...
        lines.push(`  ${fundEmoji[f]} ${fundLabels[f]}: *${posLabel} ${fs.positivity}%* | *ER ${fs.errorRate}%* | *NE ${fs.netEfficiency}%* (${fs.total})`);
      }
      lines.push('');
    }
  }
